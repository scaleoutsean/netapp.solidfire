...
class ElementSWBackup(object):
    ...
    def create_backup(self):
        ...
        try:
            ...
            self.module.fail_json(
                msg="Error starting bulk write on destination cluster", exception=to_native(err))
        ...
        try:
            read_obj = self.src_connection.start_bulk_volume_read(self.module.params["src_volume_id"],
                                                                  ...
                                                                  script_parameters=self.module.params[
                                                                      "script_parameters"],
                                                                  attributes=self.attributes)
        except solidfire.common.ApiServerError as err:
            self.module.fail_json(
                msg="Error starting bulk read on source cluster", exception=to_native(err))
        ...
        while completed is not True:
            ...
            try:
                ...
                self.module.fail_json(
                    msg="Unable to check backup job status", exception=to_native(err))
