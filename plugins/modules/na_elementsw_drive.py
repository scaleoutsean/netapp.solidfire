...
class ElementSWDrive(object):
    ...
    def __init__(self):
        ...
        self.argument_spec.update(dict(
            state=dict(required=False, choices=[
                       'present', 'absent', 'clean'], default='present'),
            drive_ids=dict(required=False, type='list',
                           elements='str', aliases=['drive_id']),
            node_ids=dict(required=False, type='list',
                          elements='str', aliases=['node_id']),
        ...
        if HAS_SF_SDK is False:
            ...
            self.sfe = netapp_utils.create_sf_connection(
                module=self.module, timeout=120)
    ...
    def get_node_id(self, node_id):
        ...
        for current_node in self.list_nodes.nodes:
            if node_id == str(current_node.node_id):
                ...
                return current_node.node_id
        self.module.fail_json(
            msg='unable to find node for node_id=%s' % node_id)
        ...
        for drive in self.all_drives.drives:
            ...
            if node_num_ids is None or drive.node_id in node_num_ids:
                if drive.status in ['active', 'failed']:
                    ...
                    self.other_drives[drive.serial] = (
                        drive.drive_id, drive.status)
    ...
    def get_drive_id(self, drive_id, node_num_ids):
        ...
        for drive in self.all_drives.drives:
            ...
            self.module.fail_json(
                msg='unable to find drive for drive_id=%s.  Debug=%s' % (drive_id, self.debug))
        if node_num_ids and drive.node_id not in node_num_ids:
            self.module.fail_json(msg='drive for drive_id=%s belongs to another node, with node_id=%d.  Debug=%s' % (
                drive_id, drive.node_id, self.debug))
    ...
    def get_available_drives(self, drives, action):
        ...
        for drive_id, drive_status in drives:
            if drive_status == 'available':
                ...
                self.module.fail_json(
                    msg='Error - cannot %s drive while it is being removed.  Debug: %s' % (action, self.debug))
            elif drive_status == 'erasing' and action == 'add':
                self.module.fail_json(
                    msg='Error - cannot %s drive while it is being erased.  Debug: %s' % (action, self.debug))
            else:
                self.module.fail_json(
                    msg='Error - cannot %s drive while it is in %s state.  Debug: %s' % (action, drive_status, self.debug))
        return action_list
    ...
    def apply(self):
        ...
        drives = None
        if self.node_ids:
            node_num_ids = [self.get_node_id(node_id)
                            for node_id in self.node_ids]

        self.get_drives_listby_status(node_num_ids)
        if self.drive_ids:
            drives = [self.get_drive_id(drive_id, node_num_ids)
