...
class ElementSWInfo(object):
    ...
    def __init__(self):
        ...
        self.argument_spec.update(dict(
            gather_subsets=dict(type='list', elements='str', aliases=[
        ...
        for role, port in [('node', 442), ('cluster', 443)]:
            try:
                conn = netapp_utils.create_sf_connection(
                    module=self.module, raise_on_connection_error=True, port=port)
                if role == 'node':
                    self.sfe_node = conn
                else:
                ...
                if str(exc) == "Bad Credentials":
                    ...
                    msg += '%s reported: %s' % ('Node' if port ==
                                                442 else 'Cluster', repr(exc))
                else:
                    msg = 'Failed to create connection for %s:%d - %s' % (
                        self.parameters['hostname'], port, repr(exc))
                self.module.fail_json(msg=msg)
            except Exception as exc:
                self.module.fail_json(msg='Failed to connect for %s:%d - %s' %
                                      (self.parameters['hostname'], port, repr(exc)))
    ...
    def filter_records(self, records, filter_dict):
        ...
        if self.parameters['fail_on_record_not_found'] and len(matched) == 0:
            msg = 'Error: no match for %s out of %d records' % (
                repr(self.parameters['filter']), len(records))
    ...
    def apply(self):
        ...
        if any(x in self.parameters['gather_subsets'] for x in my_subsets) and len(self.parameters['gather_subsets']) > 1:
            msg = 'When any of %s is used, no other subset is allowed' % repr(
                my_subsets)
            self.module.fail_json(msg=msg)
