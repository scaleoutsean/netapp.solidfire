...
class ElementSWCluster(object):
    ...
    def __init__(self):
        ...
        self.argument_spec.update(dict(
            ...
            cluster_admin_password=dict(
                required=False, type='str', no_log=True),
            ...
            fail_if_cluster_already_exists_with_larger_ensemble=dict(
        ...
        self.cluster_admin_username = input_params['username'] if input_params.get(
            'cluster_admin_username') is None else input_params['cluster_admin_username']
        self.cluster_admin_password = input_params['password'] if input_params.get(
            'cluster_admin_password') is None else input_params['cluster_admin_password']
        self.fail_if_cluster_already_exists_with_larger_ensemble = input_params[
            'fail_if_cluster_already_exists_with_larger_ensemble']
        ...
        for role, port in [('node', 442), ('cluster', 443)]:
            try:
                ...
                conn = netapp_utils.create_sf_connection(
                    module=self.module, raise_on_connection_error=True, port=port, timeout=input_params['timeout'])
                if role == 'node':
                    self.sfe_node = conn
                else:
        ...
        if self.attributes is not None:
            self.attributes.update(self.elementsw_helper.set_element_attributes(
    ...
    def get_node_cluster_info(self):
        ...
        try:
            ...
            self.debug.append("port: %s, %s" %
                              (str(self.sfe_node._port), repr(exc)))
            return None
    ...
    def check_cluster_exists(self):
        ...
        if extra_ensemble_nodes:
            self.debug.append("Extra ensemble nodes: %s" %
                              extra_ensemble_nodes)
        nodes_not_in_ensemble = requested_nodes - current_ensemble_nodes
        if nodes_not_in_ensemble:
            self.debug.append(
                "Extra requested nodes not in ensemble: %s" % nodes_not_in_ensemble)
        return True
    ...
    def create_cluster_api(self, options):
        ...
        '''
        extra_options = ['enableSoftwareEncryptionAtRest',
    ...
    def create_cluster(self):
        ...
        try:
            ...
            if 'xClusterAlreadyCreated' not in str(exc.message):
                self.module.fail_json(msg='Error creating cluster %s' % to_native(
                    exc), exception=traceback.format_exc())
    ...
    def apply(self):
        ...
        self.module.exit_json(
            changed=changed, msg=result_message, debug=self.debug)
