...
class ElementOSSnapshot(object):
    ...
    def get_account_id(self):
        ...
        try:
            ...
            self.module.fail_json(msg="Error: account_id %s does not exist" %
                                  self.account_id, exception=to_native(err))
    ...
    def apply(self):
        ...
        if self.get_src_volume_id() is None:
            self.module.fail_json(
                msg="Volume id not found %s" % self.src_volume_id)
        ...
        if snapshot_detail:
            if self.properties_provided:
                if self.expiration_time != snapshot_detail.expiration_time:
                    changed = True
                # To preserve value in case  parameter expiration_time is not defined/provided.
                else:
                ...
                if self.enable_remote_replication != snapshot_detail.enable_remote_replication:
                    changed = True
                # To preserve value in case  parameter enable_remote_Replication is not defined/provided.
                else:
                ...
                if self.snap_mirror_label != snapshot_detail.snap_mirror_label:
                    changed = True
                # To preserve value in case  parameter snap_mirror_label is not defined/provided.
                else:
