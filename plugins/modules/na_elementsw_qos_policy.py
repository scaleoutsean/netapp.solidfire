...
class ElementSWQosPolicy(object):
    ...
    def __init__(self):
        ...
        self.attributes = self.elementsw_helper.set_element_attributes(
            source='na_elementsw_qos_policy')
    ...
    def update_qos_policy(self, qos_policy_id, modify, name=None):
        ...
        try:
            ...
            self.module.fail_json(msg="Error updating qos policy: %s: %s" %
                                  (self.parameters['from_name'] if name is not None else self.parameters['name'], to_native(
                                      exc)),
    ...
    def apply(self):
        ...
        if cd_action == 'create' and self.parameters.get('from_name') is not None:
            ...
            if from_qos_policy is None:
                self.module.fail_json(
                    msg="Error renaming qos policy, no existing policy with name/id: %s" % self.parameters['from_name'])
            cd_action = 'rename'
            qos_policy_id = from_qos_policy['qos_policy_id']
            self.na_helper.changed = True
            modify = self.na_helper.get_modified_attributes(
                from_qos_policy, self.parameters)
        if cd_action == 'create' and 'qos' not in self.parameters:
            self.module.fail_json(
                msg="Error creating qos policy: %s, 'qos:' option is required" % self.parameters['name'])
        self.debug['modify'] = modify

        if not self.module.check_mode:
            if cd_action == 'create':
                self.create_qos_policy(
                    self.parameters['name'], self.parameters['qos'])
            elif cd_action == 'delete':
                self.delete_qos_policy(qos_policy_id)
            elif cd_action == 'rename':
                self.update_qos_policy(
                    qos_policy_id, modify, name=self.parameters['name'])
            elif modify:
